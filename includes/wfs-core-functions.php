...
function wfs_check_store_closed() {
  ...
  $is_store_open_always = get_option( '_wfs_store_open_always', false );

  $response = false;

  if ( $is_store_open_always !== 'yes' ) {
...
function wfs_get_term_choice( $term_id ) {
  ...
  if ( ! empty( $term_id ) ) {
    ...
    $choice = empty( $choice ) ? 'single' : strtolower( $choice );
...
/**
 * Generate available times of Delivery and Pickup
...
function wfs_get_store_timing( $service_type ) {
  ...
  $store_open_time  =  ( $is_store_open_always == 'yes' ) ? '12:00:00am' : $store_open_time;
  $store_close_time =  ( $is_store_open_always == 'yes' ) ? '11:59:59pm' : $store_close_time;
...
function wfs_get_cart_style() {
  $desktop_cart_style = get_option( '_wfs_cart_style' );
  $desktop_cart_style = !empty( $desktop_cart_style ) ? $desktop_cart_style : 'bottom';
  $mobile_cart_style  = get_option( '_wfs_mobile_cart_style' );
  $mobile_cart_style  = !empty( $mobile_cart_style ) ? $mobile_cart_style : $desktop_cart_style;
  $cart_style         = wp_is_mobile() ? $mobile_cart_style : $desktop_cart_style;
